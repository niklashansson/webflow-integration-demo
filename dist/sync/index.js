...
async function buildCategoryIdMap(validDbIds) {
...
async function buildCityIdMap() {
    ...
    const map = new Map();
    for (const item of items.items ?? []) {
        const cityName = item.fieldData?.name;
        if (cityName && item.id) {
...
export async function syncAll() {
    ...
    console.log(`   ✅ Created: ${categoryResults.created}, Updated: ${categoryResults.updated}\n`);
    // Step 2: Cities
    console.log("🏙️ Step 2: Syncing cities...");
    const cityResults = await syncAllCities(cities);
    console.log(`   ✅ Created: ${cityResults.created}, Updated: ${cityResults.updated}\n`);
    // Step 3: Build ID mappings
    console.log("🔗 Step 3: Building ID mappings...");
    const categoryIdMap = await buildCategoryIdMap(new Set(categories.map((c) => c.id)));
    const cityIdMap = await buildCityIdMap();
    console.log(`   ✅ Mapped ${categoryIdMap.size} categories, ${cityIdMap.size} cities\n`);
    // Step 4: Studios
    console.log("🏢 Step 4: Syncing studios...");
    const studioResults = await syncAllStudios(config.siteId, studios, categoryIdMap, cityIdMap);
    console.log(`   ✅ Created: ${studioResults.created}, Updated: ${studioResults.updated}\n`);
    // Step 5: Cleanup
    console.log("🧹 Step 5: Cleaning up orphaned items...");
    const cleanupResults = await cleanupAllCollections(new Set(categories.map((c) => c.id)), new Set(cities.map((c) => c.id)), new Set(studios.map((s) => s.id)));
    const totalDeleted = cleanupResults.categories.deleted +
