...
export async function syncCategoryToWebflow(siteId, category) {
    const collection = await findCollectionBySlug(siteId, "categories");
    if (!collection)
        throw new Error("Categories collection not found");
    const existingItem = await getItemByExternalId(collection.id, category.id, category.slug);
    const fieldData = {
        name: category.name,
        slug: category.slug,
...
export async function syncAllCategories(categories) {
    const results = {
        created: 0,
        updated: 0,
        failed: 0,
