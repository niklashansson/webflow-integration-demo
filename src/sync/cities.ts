...
import type { CityCollectionItem } from "../webflow/schemas.js";
import type { City } from "../db/index.js";

const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));

// Sync a single city to Webflow CMS
export async function syncCityToWebflow(siteId: string, city: City) {
  ...
  const fieldData: CityCollectionItem = {
    name: city.name,
    slug: city.slug,
    "external-id": city.id,
  };
...
export async function syncAllCities(cities: City[]) {
  const results = {
    created: 0,
    updated: 0,
    failed: 0,
    errors: [] as { cityId: string; error: string }[],
  };
